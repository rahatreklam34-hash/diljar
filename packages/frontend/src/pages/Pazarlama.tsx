...
export default function Pazarlama() {
  ...
  if (!d) return <div className="p-6 flex justify-center"><span className="w-7 h-7 border-2 border-slate-200 border-t-emerald-500 rounded-full animate-spin" /></div>;

  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-emerald-100 flex items-center justify-center"><Megaphone className="text-emerald-600" size={22} /></div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Kpi icon={Eye} cls="bg-emerald-100 text-emerald-600" label="Görüntülenen Ürün" value={String(d.ozet.tekilUrun)} />
      ...
      <div className="flex gap-1 border-b border-slate-200">
        {[['ilgi', 'İlgilenip Almayanlar'], ['harcayan', 'En Çok Harcayanlar'], ['pasif', 'Pasif Müşteriler']].map(([k, t]) => (
          <button key={k} onClick={() => setTab(k)} className={`px-4 py-2.5 text-sm font-medium border-b-2 ${tab === k ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-slate-500'}`}>{t}</button>
        ))}
      </div>
      ...
      {sms.open && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50" onClick={() => setSms({ ...sms, open: false })}>
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-white rounded-2xl p-5 space-y-3">
            ...
            <div className="bg-slate-50 rounded-xl p-3 text-sm flex items-center justify-between"><span className="text-slate-500">{sms.nums.length} alıcı</span><button onClick={kopyala} className="text-emerald-600 inline-flex items-center gap-1 text-xs"><Copy size={13} /> Numaraları Kopyala</button></div>
            ...
            <button onClick={gonder} disabled={sending} className="w-full bg-emerald-600 text-white py-2.5 rounded-xl font-medium inline-flex items-center justify-center gap-2 hover:bg-emerald-700 disabled:opacity-60"><Send size={16} /> {sending ? 'Gönderiliyor...' : 'NetGSM ile Gönder'}</button>
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-4 space-y-4">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400">Sipariş durumu değişince müşteriye otomatik SMS gönderilir. Değişkenler: <code className="text-emerald-500">{'{ad} {no} {tutar} {kargo} {takip} {firma} {kullaniciadi} {durum} {beden} {urun} {kod} {instagram} {sepet}'}</code></p>
          </div>
        </div>
      ...
      {rows.map((row) => (
        <div key={row.k} className="border border-slate-100 rounded-xl p-3">
          ...
          <textarea value={s[row.tk] || ''} onChange={(e) => set(row.tk, e.target.value)} rows={2} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-emerald-200" />
        </div>
      ))}
      <button onClick={save} disabled={busy} className="bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-emerald-700 disabled:opacity-60">{busy ? 'Kaydediliyor...' : 'Bildirim Ayarlarını Kaydet'}</button>
    </div>
  );
}
